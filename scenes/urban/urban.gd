extends Node3D
## Independent local scene using resident terrain and building assets. Google mini-map is optional.
...
const SPEED_PROFILE_SCRIPT := preload("res://shared/speed_profile.gd")
...
const DISPATCH_PLAN_SCRIPT := preload("res://app/dispatch/dispatch_plan.gd")
const DISPATCH_BOARD_SCRIPT := preload("res://shared/dispatch_board.gd")
...
var wheel_inspection := preload("res://shared/wheel_inspection.gd").new()
...
var camera_guard := preload("res://shared/camera_obstacle_guard.gd").new()
var camera_collisions_ready := false
var camera_collision_setup_started := false
var ground_track := preload("res://shared/ground_track.gd").new()
var _conflict_points := PackedVector3Array()
var _conflict_removed_meshes := 0
# 调度方案运行状态
var dispatch_plan: Dictionary = {}
var dispatch_schedule: Dictionary = {}
var dispatch_clock := 0.0
var dispatch_active := false
var dispatch_finished := false
var dispatch_board: Control
...
func _ready() -> void:
	...
	if route.size() < 2 or assets.is_empty():
		_fail("本地线路或地形资源为空。")
		return
	route = preload("res://shared/route_smoothing.gd").smooth(route)
	...
	for asset in assets:
		...
		if file == null or file.get_length() != int(asset.get("bytes", -1)):
			...
			return
		file.close()
	train = load("res://shared/train_visual.gd").new()
	add_child(train)
...
func _setup_dispatch() -> void:
	# 自动化的离线测试 / 截图 / 基准模式不启用调度，保持原有恒定速度行为。
	for flag in ["--offline-test", "--offline-capture", "--offline-benchmark"]:
	...
	if plan.is_empty() or str(plan.get("module", "")) != "urban":
		return
	var schedule := DISPATCH_PLAN_SCRIPT.build_schedule(plan, distances[-1])
	...
	var samples_x: PackedFloat64Array = dispatch_schedule.get("samples_x", PackedFloat64Array())
	mileage = float(samples_x[0]) if not samples_x.is_empty() else 0.0
	dispatch_board = DISPATCH_BOARD_SCRIPT.new()
	hud_layer.add_child(dispatch_board)
	dispatch_board.configure(plan, dispatch_schedule)
	dispatch_board.visible = true
	print("DISPATCH_PLAN train=", DISPATCH_PLAN_SCRIPT.train_label(plan),
		" module=", str(plan.get("module", "")),
		" run_time_s=", float(schedule.get("total_time_s", 0.0)),
		" stops=", int(schedule.get("stop_count", 0)),
		" pass=", int(schedule.get("pass_count", 0)),
		" max_speed_kmh=", float(schedule.get("max_speed_kmh", 0.0)))


func _current_speed_kmh() -> float:
	if dispatch_active and not dispatch_schedule.is_empty():
		return DISPATCH_PLAN_SCRIPT.speed_at_time(dispatch_schedule, dispatch_clock) * 3.6
	return SPEED * 3.6


func _destination_name() -> String:
	if not dispatch_schedule.is_empty():
		var stations: Array = dispatch_schedule.get("stations", [])
		if not stations.is_empty():
			var last: Dictionary = stations[stations.size() - 1]
			if bool(last.get("stop", true)):
				return str(last.get("name", "终点"))
			for index in range(stations.size() - 1, -1, -1):
				if bool(stations[index].get("stop", true)):
					return str(stations[index].get("name", "终点"))
	return "品川"

func _return_to_dispatch() -> void:
	# 返回上一级：列车调度台（并记住当前模块）。
	var context := get_node_or_null("/root/DispatchContext")
	if context != null:
		context.select_module("urban")
...
func _setup_scene() -> void:
	...
	cab_interior = load("res://shared/cab_interior.gd").new()
	cab_interior.name = "CabInterior"
	cab_interior.visible = false
	camera.add_child(cab_interior)
	hud_layer = CanvasLayer.new()
	var canvas := hud_layer
	add_child(canvas)
	cab_speedometer = load("res://shared/cab_speedometer.gd").new()
	...
	mini_map = preload("res://shared/mini_map_panel.gd").new()
	...
	loading_overlay = preload("res://scenes/urban/loading.gd").new()
...
func _process(delta: float) -> void:
	...
	if not ready_for_trip:
		...
		if not paused:
			...
			if dispatch_active:
				dispatch_clock = minf(dispatch_clock + delta,
					float(dispatch_schedule.get("total_time_s", 0.0)))
				mileage = DISPATCH_PLAN_SCRIPT.distance_at_time(dispatch_schedule, dispatch_clock)
			else:
			...
			if camera_guard.blocked:
				...
				if dispatch_active:
					dispatch_clock = maxf(dispatch_clock - delta, 0.0)
					mileage = DISPATCH_PLAN_SCRIPT.distance_at_time(dispatch_schedule, dispatch_clock)
				paused = true
				_update_position()
			...
			var finished := false
			if dispatch_active:
				finished = dispatch_clock >= float(dispatch_schedule.get("total_time_s", 0.0)) - 0.001
			else:
				finished = mileage >= distances[-1]
			if finished:
				dispatch_finished = true
				if loop_enabled:
					# 循环模式：到达终点后自动回到起点，持续运行。
					mileage = 0.0
					dispatch_clock = 0.0
					dispatch_finished = false
					loop_count += 1
...
func _refresh_ui() -> void:
	...
	var arrived := dispatch_finished if dispatch_active else mileage >= distances[-1]
	if paused:
		status = "已到达" + _destination_name() if arrived else "已暂停"
	elif loop_enabled:
	...
	var header := "东海道新干线 · 东京 → 品川 · 本地版"
	if dispatch_active:
		header = DISPATCH_PLAN_SCRIPT.train_label(dispatch_plan) + " · " + header
	label.text = header + "\n里程：%.2f / %.2f km    速度：%.0f km/h\n范围：轨道两侧各 500 m    航空影像：Z18\n资源：%d / %d    状态：%s    循环：%s\n视角：%s\n1/2/3/4/5：跟踪 / 俯视拍摄 / 跟随瞭望 / 驾驶室 / 轮对观察    Space：暂停/继续    R：重新预览    C：驾驶室视角    V：切换瞭望视角    L：切换循环\nT：调度时刻表    W：切换网格模式    Ctrl+Shift+F/G：小地图 / 提示面板" % [mileage / 1000, distances[-1] / 1000, _current_speed_kmh(), loaded, assets.size(), status, loop_text, view_text]
	...
	if dispatch_board != null:
		dispatch_board.refresh(dispatch_clock, mileage, paused, dispatch_finished)
...
func _seek_to_progress(percent: float) -> void:
	...
	if dispatch_active:
		dispatch_clock = DISPATCH_PLAN_SCRIPT.time_at_distance(dispatch_schedule, mileage)
		dispatch_finished = dispatch_clock >= float(dispatch_schedule.get("total_time_s", 0.0)) - 0.001
		if dispatch_finished:
			paused = true
	elif is_equal_approx(mileage, distances[-1]):
		paused = true
	camera_guard.blocked = false
	camera_guard.initialized = false
	_update_position()
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	if event.keycode == KEY_F and event.ctrl_pressed and event.shift_pressed and not event.alt_pressed and not event.meta_pressed:
		...
		get_viewport().set_input_as_handled()
		return
	if event.keycode == KEY_W and not event.ctrl_pressed and not event.shift_pressed and not event.alt_pressed and not event.meta_pressed:
	...
	if event.keycode == KEY_C:
		...
		if dispatch_board != null:
			dispatch_board.visible = not dispatch_board.visible

...
public class WheelRateData : MonoBehaviour
{
    private const int VAL_COUNT = 4;
    public Transform WheelRate; // 只作为一个父物体传入
    ...
    public void ShowMW()
    {
        if (m_LineChart & WheelRate & m_Toggles.Count == VAL_COUNT) return;
        ...
        {
            Transform tmp;
            tmp = Utility.FindChildRecursively(WheelRate, $"Tog{i}");
    ...
    void AddOneData()
    {
        ...
        for (var i = 0; i < VAL_COUNT; i++)
        {
            yvalue[i] = Globle.Fwr[vihicleIndex - 1][Globle.CurCount].reduction[i];
